...
NULL_index <- createDataPartition(a2$CDIFF_PRESENCE, p = 0.56, list = FALSE)

a2[NULL_index, "CDIFF_PRESENCE"] = FALSE
a2[-NULL_index, "CDIFF_PRESENCE"] = TRUE
...
#final_booster <- xgb.train(params = params, data = xgb_train, nrounds = best$nrounds)
...
perf_roc_NULL <- performance(pred_rocr, measure = "tpr", x.measure = "fpr")

# Plot the ROC curve
plot(perf_roc_NULL, col = "blue", lwd = 2)
